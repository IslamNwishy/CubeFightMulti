...
public class CustomMatchmakingRoomController : MonoBehaviourPunCallbacks
{
    ...
    [SerializeField]
    private GameObject JoinRoomPanel; //display for when in lobby
    [SerializeField]
    private GameObject StartPanel; //display for when in lobby
    ...
    public override void OnJoinedRoom()//called when the local player joins the room
    {
        ...
        JoinRoomPanel.SetActive(false); //hide the display for being in a lobby
    ...
    {
        StartPanel.SetActive(true); //hide the display for being in a lobby
